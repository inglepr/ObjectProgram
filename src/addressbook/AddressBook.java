...
public class AddressBook implements IAddress 
{
	...
	static FileSys fileSystem = new FileSys();
	static File file;
	...
	public void getFullOfPerson(String searchName) 
	{
		...
		if (index >= 0) {
			System.out.println(FileSys.getList().get(index).getFirstName() + " "
					+ FileSys.getList().get(index).getLastName());
		} else {
	...
	{
		int index = -1;//package com.bridgeit.addressbook.service;
		for (int i = 0; i < addressbook.AddressBook.fileSystem.getList().size(); i++) 
		{
			String personFirstName = 
					addressbook.AddressBook.fileSystem.getList().get(i).getFirstName();
