...
public class DoctorpesentImplementation implements DoctorAndPatientData{
   static ObjectMapper mapper = new ObjectMapper(); //create mapper 
	List<Patient> personlist = new ArrayList<>(); //create arraylist to store patients details
...
//add new patient
	public void addPatient() {
		...
		personlist.add(new Patient(id + 1, name, phone, age));
	}
	...
	*/
	public void fixAppointment() {
		...
		Doctor objtemp = (Doctor) doctorlist.stream().filter(i -> i.getId() == doctorId).collect(Collectors.toList())
				.get(0);
		Patient objtemp2 = (Patient) personlist.stream().filter(i -> i.getId() == patientId)
